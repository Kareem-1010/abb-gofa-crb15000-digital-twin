...
//     Unity.InferenceEngine.Worker decoderEngine, encoderEngine, spectroEngine;
...
//         Unity.InferenceEngine.Model decoder = Unity.InferenceEngine.ModelLoader.Load(Application.streamingAssetsPath + "/AudioDecoder_Tiny.sentis");

//         //Model decoderWithArgMax = Functional.... en tiedä onko tämä tarpeen?

//         Unity.InferenceEngine.Model encoder = Unity.InferenceEngine.ModelLoader.Load(Application.streamingAssetsPath + "/AudioEncoder_Tiny.sentis");
//         Unity.InferenceEngine.Model spectro = Unity.InferenceEngine.ModelLoader.Load(Application.streamingAssetsPath + "/LogMelSepctro.sentis");

//         decoderEngine = new Unity.InferenceEngine.Worker(decoder, backend);
//         encoderEngine = new Unity.InferenceEngine.Worker(encoder, backend);
//         spectroEngine = new Unity.InferenceEngine.Worker(spectro, backend);
...
//     void EncodeAudio() {
//         using var input = new Unity.InferenceEngine.Tensor<float>(new Unity.InferenceEngine.TensorShape(1, numSamples), data);

//         //spectroEngine.Execute(input); original, Execute ei ole olemassa
//         spectroEngine.Schedule(input);
//         var spectroOutput = spectroEngine.PeekOutput() as Unity.InferenceEngine.Tensor<float>;

//         //encoderEngine.Execute(spectroOutput); original
//         encoderEngine.Schedule(spectroOutput);
//         encodedAudio = encoderEngine.PeekOutput() as Unity.InferenceEngine.Tensor<float>;
//     }

//     void Update() {
//         if (transcribe && currentToken < outputTokens.Length - 1) {
//             using var tokensSoFar = new Unity.InferenceEngine.Tensor<int>(new Unity.InferenceEngine.TensorShape(1, outputTokens.Length), outputTokens);
...
//             var tokenPredictions = decoderEngine.PeekOutput() as Unity.InferenceEngine.Tensor<int>;
